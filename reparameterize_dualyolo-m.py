#!/usr/bin/env python3
r"""Convert dualyolo2-m training weights to a compact dualgelan2-m checkpoint.
...
Run from that repository's root:
    python reparameterize_dualyolo2_m_compact.py --weights best.pt \
        --cfg models/detect/dualgelan2-m-inference.yaml \
        --output best_dualgelan2-m-fp16.pt --verify
...
Use the accompanying dualgelan2-m-inference.yaml. The originally uploaded
dualgelan2-m.yaml contains the same auxiliary branch as the training YAML.
...
def validate_architectures(source_cfg, target_cfg):
    ...
    if len(target) != 39 or target[-1][2] != "DDetect":
        raise ValueError(
            ...
            "Use the supplied dualgelan2-m-inference.yaml; the uploaded "
            "dualgelan2-m.yaml still contains the auxiliary branch and DualDDetect."
        )
    ...
    for target_key, target_tensor in target_state.items():
        ...
        if source_key not in source_state:
            raise KeyError(
                ...
                "Use an original, unfused dualyolo2-m checkpoint."
            )
...
def main():
    ...
    parser.add_argument("--cfg", type=Path, default=Path("models/detect/dualgelan2-m.yaml"))
    parser.add_argument("--output", type=Path,
                        help="Default: <weights-stem>_dualgelan2-m-fp16.pt (or fp32).")
    ...
    if args.output is None:
        args.output = args.weights.with_name(
            f"{args.weights.stem}_dualgelan2-m-{precision_name}.pt"
        )
    ...
    if len(trained.model) != 60 or type(trained.model[-1]).__name__ != "DualDDetect":
        raise ValueError("Expected dualyolo2-m with DualDDetect at layer 59.")
    ...
    if len(model.model) != 39 or type(model.model[-1]).__name__ != "DDetect":
        raise ValueError("Expected dualgelan2-m with DDetect at layer 38.")
