...
class BaseModel(nn.Module):
    ...
    def _forward_once(self, x, profile=False, visualize=False):
        ...
        # Single-head GELAN/DDetect uses the standard YOLO output contract.
        # Keep the auxiliary wrapper only for the new dual-head implementation.
        if isinstance(self.model[-1], DDetect):
            return x
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in {
                    Conv,
                    nn.ConvTranspose2d, ADown,
                    SPPCSPC, AuxFeatFuse, FeatFuse,
