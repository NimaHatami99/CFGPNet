...
if not hasattr(Image, "ANTIALIAS"):
    # Pillow ≥ 10 compatibility for downstream libs that still reference ANTIALIAS
...
from utils.general import (LOGGER, TQDM_BAR_FORMAT, check_amp, check_dataset, check_file, intersect_dicts,
                           check_img_size, check_suffix, check_yaml, colorstr, get_latest_run,
                           increment_path, init_seeds, labels_to_class_weights,
                           methods, one_cycle, print_args, strip_optimizer,
...
from utils.torch_utils import (EarlyStopping, ModelEMA, de_parallel, select_device, smart_DDP, smart_optimizer,
                               torch_distributed_zero_first, smart_resume)
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        mloss = torch.zeros(4, device=device)  # box, cls, dfl, fus # mean losses
        if RANK != -1:
            train_loader.sampler.set_epoch(epoch)
        pbar = enumerate(train_loader)
        LOGGER.info(('\n' + '%11s' * 8) % ('Epoch', 'GPU_mem', 'box_loss', 'cls_loss', 'dfl_loss', 'fus_loss', 'Instances', 'Size'))
        if RANK in {-1, 0}:
...
def parse_opt(known=False):
    ...
    parser.add_argument('--cfg', type=str, default='models/detect/dualyolo2-c.yaml', help='model.yaml path')
