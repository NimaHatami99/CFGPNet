#!/usr/bin/env python3
r"""Convert dualyolo2-c training weights to a compact dualgelan2-c checkpoint.
...
Run from that repository's root:
    python reparameterize_dualyolo2_c_compact.py --weights best.pt \
        --cfg models/detect/dualgelan2-c-inference.yaml \
        --output best_dualgelan2-c-fp16.pt --verify

FP16 storage is the DEFAULT, matching the supplied model.half() example.
Use --fp32 to retain FP32 storage. FP16 needs two bytes per floating
parameter, versus four for FP32, plus metadata and buffers. Actual parameter
counts and file sizes are printed. Storage precision does not determine
inference execution precision.

Use the accompanying dualgelan2-c-inference.yaml. The originally uploaded
dualgelan2-c.yaml contains the same auxiliary branch as the training YAML.
The corrected inference YAML retains layers 0-37, removes auxiliary layers
38-58, and replaces source DualDDetect at 59 with DDetect at target layer 38.

Destination <- trained source:
    model.0..37.*   <- model.0..37.*
    model.38.cv2.*  <- model.59.cv4.*
    model.38.cv3.*  <- model.59.cv5.*
    model.38.dfl.*  <- model.59.dfl2.*
...
def source_key_for(target_key):
    """Map retained layers directly and select the trained main detection head."""
    ...
    if index == 38:
        ...
        if branch in branches and separator:
            return f"model.59.{branches[branch]}.{rest}"
...
def validate_architectures(source_cfg, target_cfg):
    """Require the 60-layer training graph and its 39-layer main-branch graph."""
    source = source_cfg["backbone"] + source_cfg["head"]
    target = target_cfg["backbone"] + target_cfg["head"]
    if len(source) != 60:
        raise ValueError("Expected 60 trained layers, with DualDDetect at layer 59.")
    if len(target) != 39 or target[-1][2] != "DDetect":
        raise ValueError(
            "Expected 39 inference layers with DDetect at layer 38. "
            "Use the supplied dualgelan2-c-inference.yaml; the uploaded "
            "dualgelan2-c.yaml still contains the auxiliary branch and DualDDetect."
        )
    if source[-1][2] != "DualDDetect":
        raise ValueError("Expected DualDDetect at source layer 59.")
    ...
    for index in range(38):
        if target[index] != source[index]:
            raise ValueError(
                f"Layer {index} differs in definition or input connections. "
                "Match the C/M checkpoint to the corresponding inference YAML."
            )
        refs = target[index][0]
        refs = refs if isinstance(refs, list) else [refs]
        resolved = [r if r >= 0 else index + r for r in refs]
        if any(r >= index or r < -1 for r in resolved):
            raise ValueError(f"Invalid retained-graph input connection at layer {index}.")

    if source[-1][0] != [52, 55, 58, 31, 34, 37]:
        raise ValueError("Unexpected auxiliary/main detection inputs in source YAML.")
    if target[-1][0] != [31, 34, 37]:
        raise ValueError("Inference detection inputs must be main features [31, 34, 37].")
    if target[-1][0] != source[-1][0][3:]:
        raise ValueError("Inference detection inputs must select the main branch.")
    if source[-1][1] != 1 or target[-1][1] != 1:
        raise ValueError("Expected one detection head instance in each architecture.")
    ...
    for target_key, target_tensor in target_state.items():
        ...
        if source_key not in source_state:
            raise KeyError(
                ...
                "Use an original, unfused dualyolo2-c checkpoint."
            )
...
def main():
    ...
    parser.add_argument("--cfg", type=Path, default=Path("models/detect/dualgelan2-c.yaml"))
    parser.add_argument("--output", type=Path,
                        help="Default: <weights-stem>_dualgelan2-c-fp16.pt (or fp32).")
    ...
    if args.output is None:
        args.output = args.weights.with_name(
            f"{args.weights.stem}_dualgelan2-c-{precision_name}.pt"
        )
    ...
    if len(trained.model) != 60 or type(trained.model[-1]).__name__ != "DualDDetect":
        raise ValueError("Expected dualyolo2-c with DualDDetect at layer 59.")
    ...
    if len(model.model) != 39 or type(model.model[-1]).__name__ != "DDetect":
        raise ValueError("Expected dualgelan2-c with DDetect at layer 38.")
